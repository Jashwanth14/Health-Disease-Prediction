...
st.markdown(
    ...
    <h1>Health Disease Prediction System</h1>
    """,
...
if st.session_state['prediction_started']:
    ...
    selected = st.selectbox('', ['Select a prediction task', 'Diabetes Prediction', 'Heart Disease Prediction', "Parkinson's Prediction"])

    # Perform action based on user selection
    if selected != 'Select a prediction task':
